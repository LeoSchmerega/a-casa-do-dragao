...
function configurarAplicacao() {
  ...
  mm.add("(max-width: 1100px)", () => {
    ...
    const secoesGerais = gsap.utils
      .toArray(".secao-animada")
      .filter(
        (secao) =>
  ...
  const setupTimeline = () => {
    ...
    if (fallbackImg) {
      gsap.to(fallbackImg, {
        ...
        ease: "power2.out",
        onComplete: () => {
          gsap.set(fallbackImg, { display: "none" });
        },
      });
    }
    ...
    const splitTitulo = new SplitText(".lealdade-conteudo h1", {
      type: "words,chars",
    });
    const splitTexto = new SplitText(".lealdade-conteudo p", { type: "words" });
    const splitSubtitulo = new SplitText(".lealdade-conteudo h2", {
      type: "words,chars",
    });
...
function inicializarPersonagensDesktop(splitInstances) {
  ...
  todosPersonagens.forEach((secaoAtual) => {
    ...
    if (fundo && conteudo) {
      const textosParaSplitar = conteudo.querySelectorAll(
        "h1, h2, p, [class*='-titulo'], [class*='-citação']",
      );
      ...
      tlPersonagens
        ...
        .from(
          cardFundo,
          { scale: 0.8, duration: 5, ease: "power4.out" },
          "<0.15",
        )
        .to(cardFundo, { autoAlpha: 1, duration: 4.5, ease: "power4.out" }, "<")
        .from(
          cardGlass,
          { scale: 0.9, duration: 5, ease: "power4.out" },
          "<0.1",
        )
        .to(cardGlass, { autoAlpha: 1, duration: 4.5, ease: "power4.out" }, "<")
        .from(
          imagem,
          { x: -45, z: -50, duration: 5, ease: "power3.out" },
          "-=0.5",
        )
      ...
      if (targaryenVerde) {
        tlPersonagens
          .from(
            targaryenVerde,
            { scale: 0.7, z: -100, duration: 1.5, ease: "power4.out" },
            "-=0.5",
          )
          .to(
            targaryenVerde,
            { autoAlpha: 1, duration: 1.5, ease: "power4.out" },
            "<",
          );
      } else if (targaryen) {
        tlPersonagens
      ...
      tlPersonagens
        ...
        .to(
          [card, cardFundo, cardGlass, imagem],
          {
            scale: 0.96,
            y: -15,
            autoAlpha: 0,
...
function inicializarParallaxCardsDesktop(eventCleanupCallbacks) {
  document.querySelectorAll(".caracteristica-card").forEach((card) => {
    const imagem =
      card.querySelector("[class*='-imagem']") || card.querySelector("img");
    if (!imagem) return;
...
function inicializarFooterDesktop() {
  ...
  tlFooter
    ...
    .fromTo(
      ".footer-logo img",
      { x: -80, filter: "blur(8px)", autoAlpha: 0 },
      {
        x: 0,
        filter: "blur(0px)",
        autoAlpha: 1,
    ...
    .fromTo(
      ".redes-sociais h3",
      { x: -60, filter: "blur(5px)", autoAlpha: 0 },
      {
        x: 0,
        filter: "blur(0px)",
        autoAlpha: 1,
    ...
    .fromTo(
      ".redes-sociais ul li",
      { x: -40, scale: 0.8, filter: "blur(5px)", autoAlpha: 0 },
      {
        x: 0,
        scale: 1,
        filter: "blur(0px)",
    ...
    .fromTo(
      ".footer-span p",
      { x: -50, autoAlpha: 0 },
      { x: 0, autoAlpha: 1, duration: 0.7, ease: "power1.out" },
      "-=0.2",
    );
}
...
function inicializarSecoesAnimadasDesktop() {
  ...
  secoes.forEach((secao) => {
    ...
    tl.to(
      secao,
      { opacity: 0, filter: "blur(15px)", y: -80, duration: 1 },
      "+=0.5",
    );
  });
}
...
function switchFaction(faccao, abrirComPadrao = false) {
  ...
  botoesAbas.forEach((b) =>
    b.classList.toggle("ativo", b.getAttribute("data-faction") === faccao),
  );
  ...
  personagens.forEach((personagem) => {
    ...
    itemLista.addEventListener("mouseenter", () => {
      document
        .querySelectorAll(".lista-personagens li")
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const trailers = [
    "assets/PRIMEIRA TELA/A Casa do Dragão - Temporada 3 - Teaser Trailer Dublado - HBO Max.mp4",
    "assets/PRIMEIRA TELA/A Casa do Dragão - Temporada 3 - Teaser Oficial - HBO Max.mp4",
  ];
